...
contract('MintinERC20', function (accounts) {
    ...
    it('should be able to add minters', async function () {
        ...
        await token.mint(accounts[0], 0, {from: accounts[1]});

        await token.mint(accounts[0], new BigNumber(101).mul(precision), {from: accounts[1]});
    ...
    it('test remove winter', async function () {
        await token.addMinter(accounts[1]);
        ...
        try {
            ...
            await Utils.balanceShouldEqualTo(token, accounts[0], 0)

            return;
        }

        assert.fail("mint succeed");
    });
});