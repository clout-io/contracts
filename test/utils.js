...
async function getBlock(id) {
    return new Promise((resolve, reject) => {
        web3.eth.getBlock(id, (err, block) => {
            if (err) {
                reject(err);
            }
            else {
...
async function getCurrentBlock() {
    ...
    }).then((number) => getBlock(number));
}

async function receiptShouldSucceed(result) {
    const receipt = await getTransaction(result.tx);
...
async function receiptShouldFailed(result) {
    const receipt = await getTransaction(result.tx);
...
function getEtherBalance(_address) {
    return new Promise((resolve, reject) => {
        web3.eth.getBalance(_address, (err, balance) => {
            if (err) {
                reject(err);
            }
            else {
...
async function checkEtherBalance(_address, expectedBalance) {
    return new Promise((resolve, reject) => {
        web3.eth.getBalance(_address, (err, balance) => {
            try {
...
async function sign(address, data) {
    return new Promise((resolve, reject) => {
        web3.eth.sign(address, data, (err, res) => {
            if (err) {
                reject(err);
            }
            else {
